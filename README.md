...
local imageId = "rbxassetid://75732220851513" -- ✅ imagem testada e garantida funcional
...
pcall(function() syn.protect_gui(screenGui) end) -- proteção se estiver usando Synapse, Delta etc.
screenGui.Parent = game:GetService("CoreGui")

-- Imagem maior no centro da tela
...
-- Texto "Carregando..." maior e mais abaixo
local textLabel = Instance.new("TextLabel")
textLabel.Size = UDim2.new(0, 500, 0, 70)
textLabel.Position = UDim2.new(0.5, -250, 0.5, 280)
...
-- Função que carrega o hub e os scripts após a tela de carregamento
local function loadHub()
    local ReplicatedStorage = game:GetService("ReplicatedStorage") -- <--- Adicione esta linha!
    local RE = ReplicatedStorage:FindFirstChild("RE")
    if not RE then return end

    -- Função auxiliar para disparar eventos remotos
    ...
    -- Define o nome "Coquette Hub 💖" com cor rosa escuro
    local argsName = {
        [1] = "RolePlayName",
        [2] = "Coquette Hub "
    }
...
local Tab13 = Window:MakeTab({"configuração", "cog"})
...
local Section = Tab1:AddSection({"versao do Hub 3.5"})
...
-- Tabela de avatares por parte do corpo
AvatarManager.Avatares = {
    PernaDireita = {
        { Nome = "Perna Direita 1", ID = 376532000 },
        { Nome = "Perna Direita 2", ID = 123456789 }, -- Adicione mais IDs conforme necessário
        -- Adicione mais opções para perna direita
    },
    PernaEsquerda = {
        { Nome = "Perna Esquerda 1", ID = 376531012 },
        { Nome = "Perna Esquerda 2", ID = 123456790 },
        -- Adicione mais opções para perna esquerda
    },
    BracoDireito = {
        { Nome = "Braço Direito 1", ID = 376530220 },
        { Nome = "Braço Direito 2", ID = 123456791 },
        -- Adicione mais opções para braço direito
    },
    BracoEsquerdo = {
        { Nome = "Braço Esquerdo 1", ID = 376531703 },
        { Nome = "Braço Esquerdo 2", ID = 123456792 },
        -- Adicione mais opções para braço esquerdo
    },
    Torso = {
        { Nome = "Torso 1", ID = 376531300 },
        { Nome = "Torso 2", ID = 123456793 },
        -- Adicione mais opções para torso
    },
    Cabeca = {
        { Nome = "Cabeça 1", ID = 0 },
        { Nome = "Cabeça 2", ID = 123456794 },
        -- Adicione mais opções para cabeça
    }
}

-- Função para obter os nomes dos avatares para o dropdown por parte do corpo
function AvatarManager:GetAvatarNames(parte)
    local nomes = {}
    for _, avatar in ipairs(self.Avatares[parte]) do
        table.insert(nomes, avatar.Nome)
    end
    return nomes
end

-- Função para obter o ID de um avatar pelo nome e parte
function AvatarManager:GetAvatarID(parte, avatarName)
    for _, avatar in ipairs(self.Avatares[parte]) do
        if avatar.Nome == avatarName then
            return avatar.ID
        end
...
-- Função para equipar todos os avatares selecionados
function AvatarManager:EquiparTodosAvatares(selecoes)
    local args = {
        {
            selecoes.PernaDireita and self:GetAvatarID("PernaDireita", selecoes.PernaDireita) or 0,
            selecoes.PernaEsquerda and self:GetAvatarID("PernaEsquerda", selecoes.PernaEsquerda) or 0,
            selecoes.BracoDireito and self:GetAvatarID("BracoDireito", selecoes.BracoDireito) or 0,
            selecoes.BracoEsquerdo and self:GetAvatarID("BracoEsquerdo", selecoes.BracoEsquerdo) or 0,
            selecoes.Torso and self:GetAvatarID("Torso", selecoes.Torso) or 0,
            selecoes.Cabeca and self:GetAvatarID("Cabeca", selecoes.Cabeca) or 0
        }
    }
    
    if not next(selecoes) then
        self:MostrarNotificacao("Nenhum avatar selecionado!")
        return
    end

    local success, result = pcall(function()
        return self.ReplicatedStorage:WaitForChild("Remotes"):WaitForChild("ChangeCharacterBody"):InvokeServer(unpack(args))
    end)
    
    if success then
        self:MostrarNotificacao("Todos os avatares equipados com sucesso!")
    else
        self:MostrarNotificacao("Falha ao equipar os avatares!")
    end
end
...
-- Dropdown para Perna Direita
local PernaDireitaDropdown = Tab3:AddDropdown({
    Name = "Perna Direita",
    Description = "Selecione para equipar",
    Default = nil,
    Options = AvatarManager:GetAvatarNames("PernaDireita"),
    Callback = function(avatarSelecionado)
        _G.AvatarSelecoes.PernaDireita = avatarSelecionado
    end
})

-- Dropdown para Perna Esquerda
local PernaEsquerdaDropdown = Tab3:AddDropdown({
    Name = "Perna Esquerda",
    Description = "Selecione para equipar",
    Default = nil,
    Options = AvatarManager:GetAvatarNames("PernaEsquerda"),
    Callback = function(avatarSelecionado)
        _G.AvatarSelecoes.PernaEsquerda = avatarSelecionado
    end
})

-- Dropdown para Braço Direito
local BracoDireitoDropdown = Tab3:AddDropdown({
    Name = "Braço Direito",
    Description = "Selecione para equipar",
    Default = nil,
    Options = AvatarManager:GetAvatarNames("BracoDireito"),
    Callback = function(avatarSelecionado)
        _G.AvatarSelecoes.BracoDireito = avatarSelecionado
    end
})

-- Dropdown para Braço Esquerdo
local BracoEsquerdoDropdown = Tab3:AddDropdown({
    Name = "Braço Esquerdo",
    Description = "Selecione para equipar",
    Default = nil,
    Options = AvatarManager:GetAvatarNames("BracoEsquerdo"),
    Callback = function(avatarSelecionado)
        _G.AvatarSelecoes.BracoEsquerdo = avatarSelecionado
    end
})

-- Dropdown para Torso
local TorsoDropdown = Tab3:AddDropdown({
    Name = "Torso",
    Description = "Selecione para equipar",
    Default = nil,
    Options = AvatarManager:GetAvatarNames("Torso"),
    Callback = function(avatarSelecionado)
        _G.AvatarSelecoes.Torso = avatarSelecionado
    end
})

-- Dropdown para Cabeça
local CabecaDropdown = Tab3:AddDropdown({
    Name = "Cabeça",
    Description = "Selecione para equipar",
    Default = nil,
    Options = AvatarManager:GetAvatarNames("Cabeca"),
    Callback = function(avatarSelecionado)
        _G.AvatarSelecoes.Cabeca = avatarSelecionado
    end
})

-- Botão para equipar todos os avatares selecionados
Tab3:AddButton({
    Name = "Equipar Todos",
    Description = "Equipa todos os avatares selecionados",
    Callback = function()
        AvatarManager:EquiparTodosAvatares(_G.AvatarSelecoes)
    end
})
...
createSoundDropdown("Selecione um meme", {
    ["Memes"] = {
        ...
        {name = "gemido", id = "105263704862377"},
        {name = " gemido", id = "92186909873950"},
...
createMusicDropdown("Phonk", {
    ["phonk"] = {
        ...
        {name = " Bala Trafika FUNK", id = "137179635212128"},
...
local ToggleLagLaptop = Tab10:AddToggle({
...
local ToggleLagExtintor = Tab10:AddToggle({
...
local loopConexaoCelular = nil

local function encontrarCelular()
    local workspaceCom = workspace:FindFirstChild("WorkspaceCom")
    if workspaceCom then
        local commercialStores = workspaceCom:FindFirstChild("001_CommercialStores")
        if commercialStores then
            local commercialStorage1 = commercialStores:FindFirstChild("CommercialStorage1")
            if commercialStorage1 then
                local store = commercialStorage1:FindFirstChild("Store")
                if store then
                    local tools = store:FindFirstChild("Tools")
                    if tools then
                        local children = tools:GetChildren()
                        if children[3] and children[3]:FindFirstChild("ClickDetector") then
                            return children[3]
                        end
                    end
                end
...
local ToggleLagCelular = Tab10:AddToggle({
    ...
    Description = "Faz spam de celulares para travar o servidor",
    Default = false
})
ToggleLagCelular:Callback(function(Value)
    lagLoopCelularAtivo = Value
    if lagLoopCelularAtivo then
        local celularObjeto = encontrarCelular()
        if not celularObjeto then
            lagLoopCelularAtivo = false
            ToggleLagCelular:Set(false)
            return
        end
        loopConexaoCelular = RunService.Heartbeat:Connect(function()
            local char = LocalPlayer.Character
            if not char or not char:FindFirstChild("HumanoidRootPart") then
                char = LocalPlayer.CharacterAdded:Wait()
            end
            if char and char:FindFirstChild("HumanoidRootPart") then
                char.HumanoidRootPart.CFrame = CFrame.new(destinoCelular)
            end
            celularObjeto = encontrarCelular()
            if celularObjeto and celularObjeto:FindFirstChild("ClickDetector") then
                pcall(function()
                    fireclickdetector(celularObjeto.ClickDetector)
                end)
            end
            wait(0.3)
        end)
    else
        if loopConexaoCelular then
            loopConexaoCelular:Disconnect()
            loopConexaoCelular = nil
        end
    end
end)
...
local ToggleLagMaca = Tab10:AddToggle({
...
local Dropdown = Tab12:AddDropdown({
...
local TeleportButton = Tab12:AddButton({
    Name = "Teleportar",
